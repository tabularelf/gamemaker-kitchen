...
A more complex jump is not inherently a better jump. You may find any of these levels of complexity to be suitable for your game and I hope you'll be able to mix and match some of these concepts to create *your* perfect jump.
...
<iframe frameborder="0" src="https://itch.io/embed-upload/11404235?color=333333" allowfullscreen="" width="1024" height="596"><a href="https://jordan-guillou.itch.io/10-levels-of-platforming-jumps">Play 10 Levels of Platforming Jumps on itch.io</a></iframe>
...
The first thing you'll want to do is create three sprites. You can do this by right-clicking the 'Sprites' folder inside of the 'Asset Browser' and selecting 'Sprite' from the 'Create' sub-menu. You can also press 'Alt-S' to make a sprite. The sprites should be called something like 'spr_player', 'spr_wall', and 'spr_bounce'.

For the player, I'm going to import two frames from Kenney's <a href="https://kenney.nl/assets/pixel-vehicle-pack" target="_blank">Pixel Vehicle Pack</a> (download below by right-clicking and saving) and then I'm setting the FPS of the animation to 4 frames-per-second.
...
This level (and the next one) are simple in a quite useful way; you don't need to calculation any collision. By not allowing the player to move horizontally while they are jumping, you know that there will be no collisions as long as the player moves up a fixed amount and moves down to the spot they jumped from.
...
A technical detail worth pointing out is the structure of our ```if``` statements. Notice how in this line

```if keyboard_check_pressed(vk_up) and place_meeting(x, y + 1, obj_collision)```

the input check is placed before the collision check. This is because if the first check is false, GameMaker won't bother processing the second check. So, because it is less costly to check input than collision, it makes sense to only run the collision check if we absolutely have to.
...
In a strict sense, these mechanics actually break the rules of the game by allowing the player to jump when they're not on the ground. However, by making the window of time where that rule can be broken very small, the actual effect is that the player doesn't notice that the rule has been broken and the game just feels more responsive to their input.
...
Then we'll change the 'Step' event to look like this:
...
Leave all of the collision code from the previous code and just make changes above that. The first thing that we do is check if the up-arrow key was pressed, and if it was, set the ```key_jump``` variable to the ```buffer``` variable. In effect, this starts the timer for how input buffering. Then to count down the timer, we just check ```if key_jump > 0``` and decrement the ```key_jump``` variable.

Next, we can check if we're on the ground and if so, set the ```coyote_time``` variable to ```coyote_time_max```. If we're not on the ground and ```coyote_time > 0```, we can decrement it. Both of these bits of code are just very simple timers which tell is whether we've recently pressed the up-arrow key or recently left the ground.

Then, we can check both of these are true by writing ```if key_jump > 0 and coyote_time > 0``` and then execute our jump. The actual jump code is the same (```vsp = -jspd```) but we also have to set ```key_jump``` to 0 here so that the game knows that the jump has been performed.

# 5. Variable jump height

<img src="http://i.imgur.com/P6bDlhy.gif" height="200"/>

This level is not the flashiest but it can completely change how a game is played. Variable jump height allows the player to perform different strength jumps depending on how long the up-arrow key is pressed. For example, if you just quickly tap the up-arrow key, you'll perform a very short jump. But if you hold the up-arrow key, you'll jump a lot higher. Obviously, this feature is present in iconic games such as Super Mario Bros but I really like the impact it has on a game like Hollow Knight where it allows you to simultaneously have very tight movement controls for platforming sections as well as really large jumps for exploration sections.

To implement this feature we just need to add one variable to the 'Create' event:

```gml
jump_decay = 0.8; // 1 = no variable jump height, lower number = smaller jumps
```

For the 'Step' event, we just need to add these four lines of code before the collision code.
...
What these lines do is check if ```vsp < 0``` (i.e. the player is travelling upwards) and the up-arrow key is not pressed. If both of these conditions are true, the current vertical velocity will be multiplied by the delay variable. This means that if you're jumping and not pressing the jump key, your vertical velocity will decrease. If you hold the jump key the entire duration of the jump though, you'll get the full height of the jump.

# 6. Bounce pads and jump types

In theory bounce pads are a simple feature; just add some vertical velocity if the player collides with a bounce pad object. But if we just add a little complexity, we can make this system a lot more versatile. By introducing the concept of 'jump_types' we can support different functionality for jumping off of different surfaces down the road. While for this tutorial I'm only adding bounce pads, in the future you could add support for very small jumps when jumping off of mud.

To support this we need to add the following code in our 'Create' event:
...
Moving onto the 'Step' event, we're going to have to make a few changes:
...
The first thing to notice is that we're adding a ```Jump()``` function. The reason for this is that we don't want to have to repeat the jump code in different places depending on whether we jumped off the ground or landed on a bounce pad. To make this even more useful, the function takes a number as an argument which represents the vertical velocity multiplier. That means that ```Jump(1)``` will execute a standard jump but ```Jump(2)``` will execute a x2 height jump.

Something else we're adding is a reference to the object we're standing on. Inside the ```place_meeting``` function we're initializing a local variable called ```_inst``` which stores a reference to the object we're colliding with. Then we can check if the ```object_index``` of that object is ```obj_bounce``` then we should automatically perform a 150% jump and set the ```jump_type``` to ```jump_types.BOUNCE```.

Otherwise, if the object is a wall, we just set the jump type to ```jump_types.NORMAL```. This basically means that now whenever we are standing on an object, we know what it is.

The last thing we have to do is change the previous jump code to the ```Jump``` function and wrap all of our previous jump code in the check ```if jump_type = jump_types.NORMAL```. The reason for the check is because we don't want to be able to perform a normal jump or run the variable jump height code if we're on a bounce pad.

Of course, if you wanted bounce pads to also have variable jump heights, you could move that code outside of the ```if``` statement.
...
The only difference between a double jump and a higher jump is that with a double jump the player can time when they want the extra height and they can jump in a different direction. So to make the most out of your double jump, you need to design levels which take advantage of those differences.

In the 'Create' event, we'll want to add these variables:
...
Our 'Step' event will look like this:
...
For the ```Jump()``` function, we need to add ```jump_capacity--``` which will take away one of our jumps as needed.

Then, the code inside the ```place_meeting``` check is mostly the same apart from the fact that - just like for coyote time - we need to set our jump capacity to the max if we're standing on the ground.

Next, initialize a local variable called ```_can_jump```. This is true if we either we are capable of performing a normal jump (i.e. on the ground) or we still have jump capacity. Then you check if the up-arrow key has been pressed and if you can jump. If so, perform a normal jump and then if we double jumped out of a bounce pad jump (i.e. ```jump_type != jump_types.NORMAL```) then we reset it back to ```NORMAL``` so that our double jump is affected by the variable jump height again.

Finally, we just have to add the check ```jump_type == jump_types.NORMAL``` to our variable jump height code since our re-jigged code doesn't already check that like in the last level.
...
To implement the wall climb, just above your ```_can_jump``` variable, initialize a ```_wall_jump``` variable. It will look like this:
...
```_wall_jump``` just checks if there's a wall to the left or right of a player. Then notice I've added that as an optional condition to our ```_can_jump``` variable so now we can either jump from the ground, in mid-air if we have a double jump, or off of a wall.
...
because you don't want to take away from the double jump ability if we've performed a wall climb.
...
Regarding changing the feel of the game, obviously changing the first variable will alter how far the player will get pushed from the wall. But the 'freeze' variables add another "dial" we can use to control the movement by stopping the player from moving back towards the wall during the wall jump.
...
Not only do these choices affect the feel of the game, they also affect what is possible for the player to do and how you construct your levels. If want the player to be able to scale walls, make ```wall_jump_spd``` and ```wall_jump_freeze_max``` smaller. If you don't, make them bigger.
...
Instead of just setting ```_wall_jump``` to ```true``` or ```false```, we need to set it to the direction in which we are touching the wall. Then, when initializing ```_can_jump```, make sure the check is ```_wall_jump != 0``` instead of just ```_wall_jump``` being ```true``` or ```false```.

The same goes for the line ```if _wall_jump``` inside of the ```if key_jump > 0 and _can_jump``` section, change that to ```if _wall_jump != 0 and !place_meeting(x, y + 1, obj_collision)```. This checks if we're touching a wall and we're not on the ground.

Then, just as before, we're incrementing ```jump_capacity``` because we don't our wall jump to count as a double jump. But here we also need to set the ```wall_jump_dir``` variable to the local variable ```_wall_jump```. This allows the game to remember what direction our wall jump is facing and won't allow us to move towards the wall for a few frames.

Then we can set ```wall_jump_freeze``` to ```wall_jump_freeze_max```. Next, set ```hsp``` (our current horizontal velocity) to ```wall_jump_spd * -wall_jump_dir``` which will push us away from the wall.
...
This level could contain a lot of detail about animations, particles, and post-processing effects but I'll just focus on a few tricks which lie within the realm of programmer art. Earlier I discussed the importance of making the player feel like the game is respecting their input. Part of visual juice is to do exactly this and provide feedback to the player about their input. Again, how far you go with juice is a matter of design and games with fewer flashy effects aren't automatically worse.
...
Now in the 'Step' event, we just need to add a few things.
...
However, returning the player object, add a normal 'Draw' event to your player object and type the following:
...
With that done, some explanation. The first line is a simple way of making the sprite face the direction in which the player is moving. ```xprevious``` is a built-in GameMaker variable which tracks the previous ```x``` value of that object. So if the player is currently at x-position 1 and their previous x-position was 0, then ```x - xprevious``` will be 1. If the result is positive, then the sprite should face right. If the result is negative, the sprite should face left.
...
The first trick is squish the player's horizontal scale when their vertical velocity is high. This adds some nice feedback to the player's vertical movement and makes every jump feel more dynamic.
...
This can make even programmer art look quite dynamic. Where that function accepts the rotation of the sprite as an argument (just after ```image_yscale```), we type ```-hsp * 2```. This just takes our horizontal velocity multiplied by 2 as the rotation of our player.
...
Next, next to the line ```coyote_time = coyote_time_max``` add this line:
...
If you've been following along this tutorial and coding, thank you for reading and hopefully you have a good understanding of how to code a basic jump as well as a complex one. Remember that it's up to you to create the perfect jump for your game.
