  ...
  for (
    const data of paginate(posts, { url, size: 24 })
      ...
      for(const index in data.results) {
        const result = search.pages(`parent=${data.results[index]?.title}`, "date=desc");
        var page = data.results[index];
        page.plugins = [];
        for (const child of paginate(result, {url, size: 24})) {
          for (const pluginData of child.results) {
            page.plugins.push(pluginData);
          }
          child.url = `/${data.results[index].url}/${child.url}`
          child.title = `${data.results[index].title} Plugins`;
          yield child;
        }
    }
  }
